...
var allParameters = [
  (rating = {
    name: "rating",
    menuDesc: "Higher Rating",
    description: "Choose the higher rated movie",
  }),
  (runtime = {
    name: "runtime",
    menuDesc: "Longer Runtime",
    description: "Choose the longer movie",
  }),
  (year = {
    name: "year",
    menuDesc: "Newer Movie",
    description: "Choose the newer movie",
  }),
  (boxOffice = {
    name: "boxOffice",
    menuDesc: "Highest Box Office",
    description: "Choose the movie with the higher box office",
  }),
];
...
var init = function () {
  ...
  if (localStorage.getItem("movieFighterHighScoreList") !== null) {
    highScoreList = JSON.parse(
      localStorage.getItem("movieFighterHighScoreList")
    );
  }
...
function checkRepeats() {
  //check for repeats is breaking the game functionality, it becomes impossible to lose
  ...
  if (pastMovies.includes(repeatObj) || pastMovies.includes(repeatObjInverse)) {
    //this if statement is broken, use a for loop?
...
function determineWinner() {
  ...
  if (
    currentMovieAObj[winningCreteriaName] >
    currentMovieBObj[winningCreteriaName]
  ) {
...
function selectMovies() {
  ...
  Promise.all([promiseA, promiseB]).then(function (PromiseVortexArray) {
    //Waits for both promises to complete before returning an array of return values
...
displayHighScores = function () {
  ...
  for (i = 0; i < sortedList.length; i++) {
    var listitem = $("<li>");
    listitem.text(
      sortedList[i].name + " : " + sortedList[i].playerScore + " points"
    );
